...
# FOR COMPUTING FACTORIAL OF 10 USING FUNCTION
...
4. compute the secret addition program with the stored secrets and another computation time secret
"""
...
async def main():
    ...
    # 4. Create secrets, add permissions, pay for and store them in the network
    # Create secrets named "a", "b", and "c" with any values, ex: 3, 4, and 5
    new_secrets = nillion.NadaValues(
        {
            "output": nillion.SecretInteger(1)
            # "one": nillion.SecretInteger(1)
            # "c": nillion.SecretInteger(5)
        }
    )

    # Set the input parties for the secrets
    party_name1 = "party1"
    # party_name2 = "party2"
    # party_name3 = "party3"
    ...
    compute_bindings.add_input_party(party_name1, party_id)
    # compute_bindings.add_input_party(party_name2, party_id)
    # compute_bindings.add_input_party(party_name3, party_id)
    compute_bindings.add_output_party(party_name1, party_id)

    # Add the secrets to be used at computation time
    computation_time_secrets = nillion.NadaValues(
        {
            # "a": nillion.SecretInteger(3),
            # "b": nillion.SecretInteger(4),
            # "limit": nillion.SecretInteger(1)
            "one": nillion.SecretInteger(1)
        }
    )
...
# # FOR COMPUTING THE LAST NUMBER OF A FIBONACCI SERIES OF LENGTH 10
...
# FOR COMPUTING (a + b + c)^3 = a^3 + b^3 + c^3 + 3(a+b)(b+c)(c+a)
